...
from utils import (makedirs, AverageMeter, save_visual, plot_loss_metrics,
                   normalize_img, normalize_tensor_imgs, testset_gt)
...
def main_worker(gpu, args):
    ...
    for epoch in range(args.num_epoch):
        ...
        if (epoch + 1) % args.eval_epoch == 0 and gpu == 0:
            ...
            # checkpointing
            checkpoint(net_frame, net_sound, history, epoch + 1, args)
...
def train(netWrapper, loader, optimizer, history, epoch, gpu, args):
    ...
    for i, batch_data in enumerate(loader):
        ...
        loss, _ = netWrapper.forward(batch_data)
        ...
        if i % args.disp_iter == 0 and gpu == 0:
            if i == 0:
                print('\n------------------------------------------------------------------------------')
            print('Epoch: [{}][{}/{}], Time: {:.2f}, Data: {:.2f}, lr_frame: {}, lr_sound: {}, loss: {:.4f}'.format(
                epoch, i, args.epoch_iters, batch_time.average(), data_time.average(),
                args.lr_frame, args.lr_sound, loss.item()))
...
def evaluate(netWrapper, loader, history, epoch, args):
    ...
    metric_output = '[Eval Summary] Epoch: {:03d}, Loss: {:.4f}, cIoU_orig_sim: {:.4f}, AUC_orig_sim: {:.4f}'.format(
        epoch, loss_meter.average(), cIoU_orig_sim, AUC_orig_sim)
    print(metric_output)
    with open(args.log, 'a') as F:
        if cIoU_orig_sim > args.best_ciou:
            F.write(
                '***************************************************************************************************\n')
...
def create_optimizer(net_frame, net_sound, args):
    ...
    params_group = [{'params': net_frame.parameters(), 'lr': args.lr_frame},
                    {'params': net_sound.parameters(), 'lr': args.lr_sound}]

    if args.optimizer == 'sgd':
        return torch.optim.SGD(params_group, momentum=args.beta1, weight_decay=args.weight_decay)
    elif args.optimizer == 'adam':
        return torch.optim.Adam(params_group, betas=(args.beta1, 0.999), weight_decay=args.weight_decay)
    elif args.optimizer == 'adamw':
        return torch.optim.AdamW(params_group, betas=(args.beta1, 0.999), weight_decay=args.weight_decay)


def checkpoint(net_frame, net_sound, history, epoch, args):
    ...
    if cur_ciou > args.best_ciou:
        ...
        torch.save(net_frame.state_dict(),
                   '{}/frame_best.pth'.format(args.ckpt))
        torch.save(net_sound.state_dict(),
                   '{}/sound_best.pth'.format(args.ckpt))
